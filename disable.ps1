##################################################
# HelloID-Conn-Prov-Target-Xedule-students-Disable
# PowerShell V2
##################################################
...
try {
    ...
    switch ($action) {
        'DisableAccount' {
            ...
            if (-not($actionContext.DryRun -eq $true)) {
                Write-Information "Disabling Xedule-students account with accountReference: [$($actionContext.References.Account)]"
                $response = Invoke-RestMethod @splatUpdateParams
                if ($response.Success -eq $false ) {
                    throw $response.Message
                }
            } else {
                Write-Information "[DryRun] Disable Xedule-students account with accountReference: [$($actionContext.References.Account)], will be executed during enforcement"
            }
    ...
    if ($($ex.Exception.GetType().FullName -eq 'Microsoft.PowerShell.Commands.HttpResponseException') -or
        ...
        $auditMessage = "Could not disable Xedule-students account. Error: $($errorObj.FriendlyMessage)"
        Write-Warning "Error at Line '$($errorObj.ScriptLineNumber)': $($errorObj.Line). Error: $($errorObj.ErrorDetails)"
    } else {
        $auditMessage = "Could not disable Xedule-students account. Error: $($_.Exception.Message)"
