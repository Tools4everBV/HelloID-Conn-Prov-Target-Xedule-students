##################################################
# HelloID-Conn-Prov-Target-Xedule-students-Delete
# PowerShell V2
##################################################
...
try {
    ...
    switch ($action) {
        'DeleteAccount' {
            ...
            if (-not($actionContext.DryRun -eq $true)) {
                Write-Information "Deleting Xedule-students account with accountReference: [$($actionContext.References.Account)]"
                $response = Invoke-RestMethod @splatDeleteParams
                if ($response.Success -eq $false ) {
                    throw $response.Message
                }
            } else {
                Write-Information "[DryRun] Delete Xedule-students account with accountReference: [$($actionContext.References.Account)], will be executed during enforcement"
            }
    ...
    if ($($ex.Exception.GetType().FullName -eq 'Microsoft.PowerShell.Commands.HttpResponseException') -or
        ...
        $auditMessage = "Could not delete Xedule-students account. Error: $($errorObj.FriendlyMessage)"
        Write-Warning "Error at Line '$($errorObj.ScriptLineNumber)': $($errorObj.Line). Error: $($errorObj.ErrorDetails)"
    } else {
        $auditMessage = "Could not delete Xedule-students account. Error: $($_.Exception.Message)"
